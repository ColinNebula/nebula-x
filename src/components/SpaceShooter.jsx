...
const SpaceShooter = () => {
  ...
  const planetScrollRef = useRef(0); // Scrolling offset for planetary backgrounds
  ...
  useEffect(() => {
    ...
    const render = (ctx, timestamp) => {
      ...
      if (currentWave >= 1 && currentWave <= 4) {
        planetTheme = 'moon'; // Waves 1-4: Moon (gray, cratered surface)
      } else if (currentWave >= 5 && currentWave <= 8) {
        planetTheme = 'mars'; // Waves 5-8: Mars (red/orange desert)
      } else if (currentWave >= 9 && currentWave <= 12) {
        planetTheme = 'jupiter'; // Waves 9-12: Jupiter (orange/brown gas bands)
      } else if (currentWave >= 13 && currentWave <= 16) {
        planetTheme = 'saturn'; // Waves 13-16: Saturn (pale yellow with rings)
      ...
      if (planetTheme) {
        ...
        // Draw planetary surface at bottom of screen
        if (planetTheme === 'moon') {
          // Moon: Gray cratered surface
          const moonGrad = ctx.createLinearGradient(0, planetY, 0, GAME_HEIGHT);
          moonGrad.addColorStop(0, 'rgba(80, 80, 100, 0.3)');
          moonGrad.addColorStop(0.5, 'rgba(60, 60, 80, 0.6)');
          moonGrad.addColorStop(1, 'rgba(40, 40, 60, 0.8)');
          ctx.fillStyle = moonGrad;
          ctx.fillRect(0, planetY, GAME_WIDTH, planetHeight);

          // Draw craters (static pattern with scroll offset)
          ctx.fillStyle = 'rgba(30, 30, 50, 0.4)';
          for (let i = 0; i < 15; i++) {
            const x = ((i * 80 - planetScrollRef.current * 0.3) % (GAME_WIDTH + 100)) - 50;
            const y = planetY + 40 + Math.sin(i * 2) * 60;
            const radius = 15 + Math.sin(i * 3) * 10;
            ctx.beginPath();
            ctx.arc(x, y, radius, 0, Math.PI * 2);
            ctx.fill();
          }
        } else if (planetTheme === 'mars') {
          // Mars: Red/orange desert with dunes
          const marsGrad = ctx.createLinearGradient(0, planetY, 0, GAME_HEIGHT);
          marsGrad.addColorStop(0, 'rgba(180, 80, 40, 0.3)');
          marsGrad.addColorStop(0.5, 'rgba(140, 60, 30, 0.6)');
          marsGrad.addColorStop(1, 'rgba(100, 40, 20, 0.8)');
          ctx.fillStyle = marsGrad;
          ctx.fillRect(0, planetY, GAME_WIDTH, planetHeight);

          // Draw sand dunes (wavy pattern)
          ctx.fillStyle = 'rgba(160, 70, 35, 0.5)';
          ctx.beginPath();
          ctx.moveTo(-50, planetY + 80);
          for (let x = -50; x < GAME_WIDTH + 50; x += 20) {
            const scrolledX = (x - planetScrollRef.current * 0.4) % (GAME_WIDTH + 100);
            const y = planetY + 80 + Math.sin((scrolledX + planetScrollRef.current) * 0.02) * 30;
            ctx.lineTo(scrolledX, y);
          }
          ...
          // Jupiter: Orange/brown gas giant with bands
          for (let band = 0; band < 8; band++) {
            const bandY = planetY + (band * 25);
            const colors = ['rgba(200, 140, 80, 0.6)', 'rgba(180, 110, 60, 0.7)', 'rgba(160, 100, 50, 0.6)'];
            const bandGrad = ctx.createLinearGradient(0, bandY, 0, bandY + 25);
            bandGrad.addColorStop(0, colors[band % 3]);
            bandGrad.addColorStop(0.5, colors[(band + 1) % 3]);
            bandGrad.addColorStop(1, colors[(band + 2) % 3]);
            ctx.fillStyle = bandGrad;

            // Wavy bands with scroll
            ctx.beginPath();
            ctx.moveTo(-10, bandY);
            for (let x = -10; x < GAME_WIDTH + 10; x += 10) {
              const scrolledX = x - (planetScrollRef.current * (0.3 + band * 0.05));
              const waveY = bandY + Math.sin((scrolledX + band * 50) * 0.015) * 8;
              ctx.lineTo(x, waveY);
            }
            ctx.lineTo(GAME_WIDTH + 10, bandY + 25);
            ctx.lineTo(-10, bandY + 25);
            ctx.closePath();
            ctx.fill();
          }

          // Great Red Spot
          const spotX = (350 - planetScrollRef.current * 0.2) % GAME_WIDTH;
          const spotY = planetY + 90;
          const spotGrad = ctx.createRadialGradient(spotX, spotY, 0, spotX, spotY, 50);
          spotGrad.addColorStop(0, 'rgba(200, 80, 60, 0.6)');
          spotGrad.addColorStop(0.7, 'rgba(160, 60, 40, 0.4)');
          spotGrad.addColorStop(1, 'rgba(0, 0, 0, 0)');
          ctx.fillStyle = spotGrad;
          ctx.beginPath();
          ctx.ellipse(spotX, spotY, 60, 35, 0, 0, Math.PI * 2);
          ctx.fill();
        } else if (planetTheme === 'saturn') {
          // Saturn: Pale yellow surface
          const saturnGrad = ctx.createLinearGradient(0, planetY, 0, GAME_HEIGHT);
          saturnGrad.addColorStop(0, 'rgba(220, 200, 140, 0.3)');
          saturnGrad.addColorStop(0.5, 'rgba(200, 180, 120, 0.6)');
          saturnGrad.addColorStop(1, 'rgba(180, 160, 100, 0.8)');
          ctx.fillStyle = saturnGrad;
          ctx.fillRect(0, planetY, GAME_WIDTH, planetHeight);

          // Saturn's rings (perspective view from side)
          for (let ring = 0; ring < 5; ring++) {
            const ringY = planetY + 30 + ring * 15;
            const ringAlpha = 0.4 - ring * 0.05;
            ctx.strokeStyle = `rgba(200, 180, 140, ${ringAlpha})`;
            ctx.lineWidth = 3 + ring;
            ctx.beginPath();
            for (let x = 0; x < GAME_WIDTH; x += 5) {
              const scrolledX = (x - planetScrollRef.current * (0.2 + ring * 0.03)) % GAME_WIDTH;
              const y = ringY + Math.sin((scrolledX + ring * 100) * 0.02) * (5 + ring * 2);
          ...
          // Uranus: Cyan/blue ice giant
          const uranusGrad = ctx.createLinearGradient(0, planetY, 0, GAME_HEIGHT);
          uranusGrad.addColorStop(0, 'rgba(120, 200, 220, 0.3)');
          uranusGrad.addColorStop(0.5, 'rgba(80, 160, 200, 0.6)');
          uranusGrad.addColorStop(1, 'rgba(60, 120, 180, 0.8)');
          ctx.fillStyle = uranusGrad;
          ctx.fillRect(0, planetY, GAME_WIDTH, planetHeight);

          // Faint atmospheric bands
          for (let band = 0; band < 6; band++) {
            const bandY = planetY + (band * 30);
            ctx.strokeStyle = `rgba(100, 180, 210, ${0.2 + band * 0.05})`;
            ctx.lineWidth = 2;
            ctx.beginPath();
            for (let x = 0; x < GAME_WIDTH; x += 5) {
              const scrolledX = (x - planetScrollRef.current * (0.25 + band * 0.02)) % GAME_WIDTH;
              const y = bandY + Math.sin((scrolledX + band * 80) * 0.01) * 5;
              if (x === 0) ctx.moveTo(x, y);
              else ctx.lineTo(x, y);
            }
            ctx.stroke();
          ...
          for (let i = 0; i < 20; i++) {
            const x = ((i * 45 - planetScrollRef.current * 0.6) % (GAME_WIDTH + 50)) - 25;
            const y = planetY + 50 + Math.sin(i * 5 + planetScrollRef.current * 0.05) * 80;
      ...
      enemyBulletsRef.current.forEach(bullet => {
        ...
        if (bullet.isAgaCannon) {
          // AGA CANNON - Massive energy blast
          ctx.save();
          ctx.translate(bullet.x, bullet.y);
          ctx.rotate(Math.atan2(bullet.vy, bullet.vx));

          const agaSize = (bullet.agaSize || 1) * 15;

          // Outer energy glow
          ctx.shadowColor = '#00ffff';
          ctx.shadowBlur = reducedBulletEffects ? 15 : 40;

          // Energy core - pulsing effect
          const pulseSize = agaSize + Math.sin(Date.now() / 50) * 3;
          const gradient = ctx.createRadialGradient(0, 0, 0, 0, 0, pulseSize);
          gradient.addColorStop(0, '#ffffff');
          gradient.addColorStop(0.2, '#00ffff');
          gradient.addColorStop(0.5, '#0088ff');
          gradient.addColorStop(0.8, '#0044ff');
          ...
          // Electric arcs around the projectile
          if (!reducedBulletEffects) {
            ctx.strokeStyle = '#88ffff';
            ctx.lineWidth = 2;
            for (let i = 0; i < 4; i++) {
              const angle = (Date.now() / 100 + i * Math.PI / 2) % (Math.PI * 2);
              const arcRadius = pulseSize * 1.3;
              ctx.beginPath();
              ctx.arc(0, 0, arcRadius, angle, angle + 0.3);
              ctx.stroke();
            }
          }
          ...
          ctx.fillStyle = '#ffffff';
          ctx.beginPath();
          ctx.arc(0, 0, agaSize * 0.4, 0, Math.PI * 2);
          ctx.fill();

          // Trailing energy
          ctx.fillStyle = 'rgba(0, 136, 255, 0.6)';
          ctx.beginPath();
          ctx.ellipse(-agaSize * 1.5, 0, agaSize, agaSize * 0.6, 0, 0, Math.PI * 2);
          ctx.fill();
      ...
      if (bossRef.current) {
        ...
        // Check if this is a major boss (5, 10, 15, 20) for special dark theme
        const isMajorBoss = [5, 10, 15, 20].includes(waveRef.current);
        ...
        if (isMajorBoss) {
          // Major bosses (5, 10, 15, 20) - Black with red highlights
          accentColor = '#ff0000';
          accentColorDim = '#aa0000';
          hullDark = '#000000';
          hullMid = '#110000';
          hullLight = '#220000';
          eyeColor = '#ff0000';
          glowColor = '#ff0000';
        } else if (zonePattern === 'slow_tank') {
        ...
        if (boss.isSuperBoss) {
        ...
        } else if (boss.isEliteBoss) {
          // ========== ELITE BOSS - WAVE 5 SPECIAL ==========
          // Larger engine array with stronger glow
          ctx.shadowColor = glowColor;
          ctx.shadowBlur = 30;
          for (let i = 0; i < 4; i++) {
            const engineY = by + bossH * (0.2 + i * 0.22);
            ctx.fillStyle = accentColor;
            ctx.beginPath();
            ctx.ellipse(bx + bossW + 15, engineY, 18, 8, 0, 0, Math.PI * 2);
            ctx.fill();
            ctx.fillStyle = '#ffffff';
            ctx.beginPath();
            ctx.ellipse(bx + bossW + 10, engineY, 8, 4, 0, 0, Math.PI * 2);
            ctx.fill();
          }
          ctx.shadowBlur = 0;

          // Main hull gradient - more aggressive
          const eliteGrad = ctx.createLinearGradient(bx, by, bx, by + bossH);
          eliteGrad.addColorStop(0, '#000000');
          eliteGrad.addColorStop(0.2, hullDark);
          eliteGrad.addColorStop(0.5, hullMid);
          eliteGrad.addColorStop(0.8, hullDark);
          eliteGrad.addColorStop(1, '#000000');
          ctx.fillStyle = eliteGrad;

          // Larger, more imposing body
          ctx.beginPath();
          ctx.moveTo(bx + 20, centerY);
          ctx.lineTo(bx + bossW * 0.25, by + 8);
          ctx.lineTo(bx + bossW * 0.7, by + 5);
          ctx.lineTo(bx + bossW, by + bossH * 0.15);
          ctx.lineTo(bx + bossW, by + bossH * 0.85);
          ctx.lineTo(bx + bossW * 0.7, by + bossH - 5);
          ctx.lineTo(bx + bossW * 0.25, by + bossH - 8);
          ctx.closePath();
          ctx.fill();
          ...
          ctx.strokeStyle = accentColor;
          ctx.lineWidth = 2;
          ctx.globalAlpha = 0.7 + Math.sin(pulsePhase) * 0.2;
          ctx.beginPath();
          ctx.moveTo(bx + bossW * 0.4, by + 8);
          ctx.lineTo(bx + bossW * 0.4, by + bossH - 8);
          ctx.moveTo(bx + bossW * 0.6, by + 5);
          ctx.lineTo(bx + bossW * 0.6, by + bossH - 5);
          ctx.stroke();
          ctx.globalAlpha = 1;
          ...
          ctx.fillStyle = hullLight;
          ctx.beginPath();
          ctx.moveTo(bx + bossW * 0.3, by + 8);
          ctx.lineTo(bx + bossW * 0.35, by - 35);
          ctx.lineTo(bx + bossW * 0.5, by - 45);
          ctx.lineTo(bx + bossW * 0.65, by - 35);
          ctx.lineTo(bx + bossW * 0.7, by + 5);
          ctx.closePath();
          ctx.fill();
          ...
          // Larger bottom wing
          ctx.beginPath();
          ctx.moveTo(bx + bossW * 0.3, by + bossH - 8);
          ctx.lineTo(bx + bossW * 0.35, by + bossH + 35);
          ctx.lineTo(bx + bossW * 0.5, by + bossH + 45);
          ctx.lineTo(bx + bossW * 0.65, by + bossH + 35);
          ctx.lineTo(bx + bossW * 0.7, by + bossH - 5);
          ctx.closePath();
          ctx.fill();
          ...
          // MASSIVE LASER CANNON - Central weapon
          const laserActive = boss.laserCharging || boss.laserFiring;
          ctx.fillStyle = '#111111';
          ctx.beginPath();
          ctx.moveTo(bx - 15, centerY);
          ctx.lineTo(bx + 45, centerY - 30);
          ctx.lineTo(bx + 45, centerY + 30);
          ctx.closePath();
          ctx.fill();
          ...
          ctx.fillStyle = laserActive ? glowColor : hullMid;
          ctx.shadowColor = laserActive ? glowColor : accentColorDim;
          ctx.shadowBlur = laserActive ? 50 + Math.sin(pulsePhase * 2) * 20 : 15;
          ctx.beginPath();
          ctx.arc(bx + 15, centerY, 22, 0, Math.PI * 2);
          ctx.fill();

          // Inner laser ring
          ctx.fillStyle = laserActive ? '#ffffff' : accentColor;
          ctx.beginPath();
          ctx.arc(bx + 15, centerY, 12, 0, Math.PI * 2);
          ctx.fill();
          ctx.shadowBlur = 0;

          // Secondary weapon pods
          for (let side = -1; side <= 1; side += 2) {
            const weaponY = centerY + side * bossH * 0.3;
            ctx.fillStyle = accentColorDim;
            ctx.shadowColor = glowColor;
            ctx.shadowBlur = 10;
            ctx.beginPath();
            ctx.arc(bx + 35, weaponY, 10, 0, Math.PI * 2);
            ctx.fill();
            ctx.fillStyle = '#ffffff';
            ctx.beginPath();
            ctx.arc(bx + 35, weaponY, 4, 0, Math.PI * 2);
            ctx.fill();
          }
          ctx.shadowBlur = 0;
          ...
          ctx.fillStyle = hullDark;
          ctx.beginPath();
          ctx.arc(bx + bossW * 0.45, centerY, 18, 0, Math.PI * 2);
          ctx.fill();
          ...
          ctx.fillStyle = eyeColor;
          ctx.beginPath();
          ctx.arc(bx + bossW * 0.45, centerY, 10, 0, Math.PI * 2);
          ctx.fill();
          ctx.fillStyle = '#ffffff';
          ctx.beginPath();
          ctx.arc(bx + bossW * 0.45 - 3, centerY - 3, 4, 0, Math.PI * 2);
          ctx.fill();
          ctx.shadowBlur = 0;
        ...
        // Draw EMP shockwave
        if (boss.empActive && (boss.isMegaBoss || boss.isEliteBoss) && boss.empRadius > 1) {
          const empX = bx + bossW / 2;
          const empY = centerY;
        ...
        // Draw Aga Cannon charging effect
        if (boss.agaCannonCharging && boss.agaCannonCharge > 0) {
          const chargePercent = boss.agaCannonCharge / 100;
          const cannonX = bx + 15;
          const cannonY = centerY;
          const chargeRadius = 10 + chargePercent * 40;

          ctx.save();

          // Outer energy ring
          ctx.strokeStyle = `rgba(0, 255, 255, ${0.4 + chargePercent * 0.6})`;
          ctx.lineWidth = 4;
          ctx.shadowColor = '#00ffff';
          ctx.shadowBlur = 20 + chargePercent * 30;
          ctx.beginPath();
          ctx.arc(cannonX, cannonY, chargeRadius + 15, 0, Math.PI * 2);
          ctx.stroke();

          // Inner energy ring
          ctx.strokeStyle = `rgba(100, 200, 255, ${0.6 + chargePercent * 0.4})`;
          ctx.lineWidth = 3;
          ctx.beginPath();
          ctx.arc(cannonX, cannonY, chargeRadius, 0, Math.PI * 2);
          ctx.stroke();

          // Energy particles
          for (let i = 0; i < 8; i++) {
            const angle = (Date.now() / 500 + i * Math.PI / 4) % (Math.PI * 2);
            const particleRadius = chargeRadius + 20;
            const px = cannonX + Math.cos(angle) * particleRadius;
            const py = cannonY + Math.sin(angle) * particleRadius;

            ctx.fillStyle = '#00ffff';
            ctx.beginPath();
            ctx.arc(px, py, 3 + chargePercent * 3, 0, Math.PI * 2);
            ctx.fill();
          }

          // Core glow
          const coreGrad = ctx.createRadialGradient(cannonX, cannonY, 0, cannonX, cannonY, chargeRadius * 0.5);
          coreGrad.addColorStop(0, `rgba(255, 255, 255, ${chargePercent})`);
          coreGrad.addColorStop(0.5, `rgba(0, 255, 255, ${chargePercent * 0.8})`);
          coreGrad.addColorStop(1, `rgba(0, 136, 255, 0)`);
          ctx.fillStyle = coreGrad;
          ctx.beginPath();
          ctx.arc(cannonX, cannonY, chargeRadius * 0.5, 0, Math.PI * 2);
          ctx.fill();
    ...
    const gameLoop = (timestamp) => {
      ...
      if (!bossActiveRef.current && !bossRef.current && waveKillsRef.current >= waveKillsNeededRef.current) {
        ...
        // Create boss
        const bossType = waveRef.current >= 15 ? 'mega' : waveRef.current >= 10 ? 'super' : waveRef.current === 5 ? 'elite' : 'normal';
        const isMegaBoss = bossType === 'mega';
        const isSuperBoss = bossType === 'super' || bossType === 'mega';
        const isEliteBoss = bossType === 'elite'; // Special wave 5 boss

        // Progressive size scaling for major bosses (5, 10, 15, 20)
        let bossScale = 1.0;
        if (waveRef.current === 5) bossScale = 1.4;
        else if (waveRef.current === 10) bossScale = 1.7;
        else if (waveRef.current === 15) bossScale = 2.0;
        else if (waveRef.current === 20) bossScale = 2.3;
        else if (isEliteBoss) bossScale = 1.4;
        else if (isSuperBoss) bossScale = 1.6;
        else if (isMegaBoss) bossScale = 1.9;

        const bossHealth = 100 + (waveRef.current * 50);
        const bossShield = (isSuperBoss || isEliteBoss) ? bossHealth * 0.3 : 0;
        ...
        bossRef.current = {
          x: GAME_WIDTH + 50,
          y: GAME_HEIGHT / 2 - (BOSS_HEIGHT * bossScale) / 2,
          width: BOSS_WIDTH * bossScale,
          height: BOSS_HEIGHT * bossScale,
          scale: bossScale,
          ...
          phase: 0,
          phaseTimer: 0,
          targetY: GAME_HEIGHT / 2 - (BOSS_HEIGHT * bossScale) / 2,
          invincible: false,
          regenerating: false,
          regenCount: 0,
          maxRegens: 2,
          ...
          laserDuration: 0,
          agaCannonCharging: false,
          agaCannonCharge: 0,
          agaCannonFiring: false,
          lastAgaCannon: Date.now(),
          lastShot: Date.now(),
          lastLaser: Date.now(),
          lastCannonShot: Date.now(),
          lastSecondaryShot: Date.now(),
          fireRate: isMegaBoss ? 1200 : isSuperBoss ? 1500 : isEliteBoss ? 1600 : 2000,
          cannonFireRate: 2500,
          secondaryFireRate: 1800,
          agaCannonCooldown: waveRef.current === 20 ? 6000 : waveRef.current === 15 ? 7000 : waveRef.current === 10 ? 8000 : 9000,
          empActive: false,
          empRadius: 0,
          empRange: isEliteBoss ? 300 : 400,
          empCooldown: isEliteBoss ? 8000 : 10000,
          empCharging: false,
          empCharge: 0,
          lastEMP: Date.now(),
          regenEMPFired: false
      ...
      if (bossRef.current) {
        ...
        if (!boss.entered) {
          ...
          // AGA CANNON - Massive charged weapon for major bosses (5, 10, 15, 20)
          const isMajorBoss = [5, 10, 15, 20].includes(waveRef.current);
          if (isMajorBoss && boss.entered && !isRegenerating && !boss.laserFiring) {
            // Start charging Aga Cannon
            if (!boss.agaCannonCharging && !boss.agaCannonFiring && currentTime - boss.lastAgaCannon > boss.agaCannonCooldown) {
              boss.agaCannonCharging = true;
              boss.agaCannonCharge = 0;
              soundSystem.playWaveCannonCharge();
              floatingTextsRef.current.push({
                x: boss.x + boss.width / 2,
                y: boss.y - 40,
                text: '⚡ AGA CANNON CHARGING ⚡',
                color: '#ffff00',
                lifetime: 60,
                vy: -0.5,
                flash: true
            ...
            // Charge up the Aga Cannon
            if (boss.agaCannonCharging) {
              boss.agaCannonCharge += 1.5;
              if (boss.agaCannonCharge >= 100) {
                boss.agaCannonCharging = false;
                boss.agaCannonFiring = true;
                soundSystem.playWaveCannonFire();
              }
            }

            // Fire the Aga Cannon
            if (boss.agaCannonFiring) {
              const player = playerRef.current;
              const cannonX = boss.x;
              const cannonY = boss.y + boss.height / 2;

              // Fire massive energy blast
              const blastCount = waveRef.current === 20 ? 5 : waveRef.current === 15 ? 4 : waveRef.current === 10 ? 3 : 2;
              for (let i = 0; i < blastCount; i++) {
                const dx = player.x + PLAYER_WIDTH / 2 - cannonX;
                const dy = player.y + PLAYER_HEIGHT / 2 - cannonY + (i - (blastCount - 1) / 2) * 40;
                const angle = Math.atan2(dy, dx);
                const speed = 7;

                enemyBulletsRef.current.push({
                  x: cannonX,
                  y: cannonY + (i - (blastCount - 1) / 2) * 40,
                  vx: Math.cos(angle) * speed,
                  vy: Math.sin(angle) * speed,
                  aimed: true,
                  isAgaCannon: true,
                  agaSize: waveRef.current / 5, // Size scales with wave
                  isBossShot: true
                });
              }

              // Massive muzzle flash
              createExplosion(cannonX + 30, cannonY, 'boss');

              boss.agaCannonFiring = false;
              boss.lastAgaCannon = currentTime;
            }
          }
          ...
          // EMP (Electromagnetic Pulse) attack for mega/super/elite boss (disabled during regen)
          if ((boss.isMegaBoss || boss.isEliteBoss) && boss.entered && !isRegenerating) {
            // Start charging EMP
      ...
      let enemyCounter = 0;
      collisionChecks = 0; // Reset, not redeclare
      enemiesRef.current = enemiesRef.current.filter(enemy => {
        try {
        // Apply time warp slowdown
        const effectiveSpeed = (enemy.speed || ENEMY_SPEED) * timeWarpModifier;
              ...
              <div className="challenge-modal-overlay">
                <div className="challenge-modal replay-modal">
                  ...
                  <div className="replay-controls">
                    ...
                    >
                      <span className="btn-icon">{isRecording ? 'ÃÂ¢ÃÂÃÂºÃÂ¯ÃÂ¸ÃÂ' : 'ÃÂÃÂ¬'}</span>
                        ...
                        <div key={replay.id} className="replay-item">
                          <div className="replay-info">
                            ...
                            <div className="replay-stats">
                              ...
                              <span>ÃÂÃÂ Wave {replay.wave}</span>
                              <span>⏱️ {Math.floor(replay.duration / 1000)}s</span>
                              <span>ÃÂÃÂ {replay.frameCount} frames</span>
                            </div>
          ...
          <div className="overlay checkpoint-overlay">
            ...
            <div className="checkpoint-content">
              ...
              <div className="checkpoint-buttons">
                ...
                >
                  <span className="btn-icon">{saveFeedback ? '✓' : 'ÃÂÃÂ¾'}</span> {saveFeedback ? 'SAVED!' : 'SAVE PROGRESS'}
                </button>
                <button
                  onClick={() => { soundSystem.playUISparkle(); setShowCustomize(true); }}
