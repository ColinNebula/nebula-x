...
class SoundSystem {
  ...
  playUISparkle() {
    try {
      const sparkleSound = new Audio(`${process.env.PUBLIC_URL}/mixkit-fairy-magic-sparkle-871.mp3`);
...
const SpaceShooter = () => {
  ...
  const playGameMusic = useCallback((trackName, loop = true) => {
    ...
    gameMusicRef.current = new Audio(`${process.env.PUBLIC_URL}/${trackName}`);
  ...
  useEffect(() => {
    ...
    if (gameState === 'menu' || gameState === 'splash') {
      ...
      if (!menuMusicRef.current) {
        menuMusicRef.current = new Audio(`${process.env.PUBLIC_URL}/Under_Cover_of_the_Myst.mp3`);
      ...
      try {
        const checkpointMusic = new Audio(`${process.env.PUBLIC_URL}/Spooky_Loop.mp3`);
  ...
  const startGame = useCallback(() => {
    ...
    try {
      const spawnSound = new Audio(`${process.env.PUBLIC_URL}/mixkit-spell-of-healing-876.wav`);
    ...
    levelFadeRef.current = { 
      ...
      fadeIn: true, 
      alpha: 1, 
      showText: 'survival' ? 5 : (gameMode === 'bossRush' ? 5 : 1),
  ...
  const loadGame = useCallback(() => {
    ...
    try {
      ...
      // Load saved data
      waveRef.current = saveData.mp3e || 1;
  ...
  const startCheckpointTransition = useCallback((boss, completedWave) => {
    ...
    try {
      const bossDeathSound = new Audio(`${process.env.PUBLIC_URL}/mixkit-explosion-hit-1704.mp3`);
    ...
    try {
      const levelCompleteSound = new Audio(`${process.env.PUBLIC_URL}/mixkit-completion-of-a-level-2063.wav`);
  ...
  const spawnEnemy = useCallback(() => {
    ...
    if (Math.random() < 0.15) {
      try {
        const spawnSound = new Audio(`${process.env.PUBLIC_URL}/mixkit-technology-alert-transition-3121.mp3`);
    ...
    if (enemy.type === 'turret') {
      ...
      enemy.health = 2;
      enemy.mp3ePhase = 0;
      enemy.mp3eAmplitude = 3; // How much bullets curve
      enemy.attackCooldown = 0;
  ...
  const spawnMiniBoss = useCallback(() => {
    ...
    const gameLoop = (timestamp) => {
      ...
      if (gamepad) {
        ...
        // O (Circle) = button 1 = Force toggle
        // Square = button 2 = Missile
        // Triangle = button 3 = Laser beam (when maxed)
        // L1 = button 4 = (unused)
        // R1 = button 5 = Shoot (alt)
        // L2 = button 6 = Wave Cannon charge
        // R2 = button 7 = Shoot (alt)
        // Share = button 8
        ...
        gpShoot = gamepad.buttons[0]?.pressed || gamepad.buttons[5]?.pressed || gamepad.buttons[7]?.pressed;
        gpMissile = gamepad.buttons[2]?.pressed;
        gpForce = gamepad.buttons[1]?.pressed;
        gpWaveCannon = gamepad.buttons[6]?.pressed; // L2 trigger
        gpLaser = gamepad.buttons[3]?.pressed; // Triangle = laser beam
        gpDash = gamepad.buttons[10]?.pressed || gamepad.buttons[11]?.pressed; // L3 or R3 = dash
        
        // Handle polarity toggle (L1 or R1 bumpers - buttons 4 and 5)
        if ((gamepad.buttons[4]?.pressed || gamepad.buttons[5]?.pressed) && !gamepadButtonsRef.current.polarity) {
          const newPolarity = polarityRef.current === 'light' ? 'dark' : 'light';
          polarityRef.current = newPolarity;
          setPolarity(newPolarity);
        }
        gamepadButtonsRef.current.polarity = gamepad.buttons[4]?.pressed || gamepad.buttons[5]?.pressed || false;
      ...
      if (trailOpt.enabled && !playerInvincibleRef.current) {
        for (let i = 0; i < trailOpt.particleCount; i++) {
          const spread = 4;
      ...
      if (engineTrailRef.current.length > 100) {
        engineTrailRef.current = engineTrailRef.current.slice(-100);
      }
        ...
        if (checkCollision(
          ...
          switch (powerup.type) {
            ...
            case SHIELD':
              ...
              const shieldSound = new Audio(`${process.env.PUBLIC_URL}/710278__dan2008__charged-up.mp3`);
      ...
      enemiesRef.current = enemiesRef.current.filter(enemy => {
        ...
        const effectiveSpeed = enemy.speed * timeWarpModifier;
        ...
        if (enemy.type === 'turret') {
          ...
          enemy.x -= enemy.speed;
          enemy.mp3ePhase += 0.08;
          enemy.y += Math.sin(enemy.mp3ePhase) * 2;
          // Keep on screen
          ...
          if (enemy.armed) {
            ...
            const dist = Math.sqrt(dx * dx + dy * dy);
            if (dist > 10) {
              enemy.x += (dx / dist) * enemy.speed + (dx * enemy.magnetStrength);
              enemy.y += (dy / dist) * enemy.speed * 0.5 + (dy * enemy.magnetStrength);
            }
          ...
          enemy.x += enemy.speed; // Move right (from left side)
        } else {
          enemy.x -= enemy.speed; // Move left (from right side)
        }
          ...
          if (enemy.type === 'turret') {
            ...
            if (enemy.x < GAME_WIDTH - 80 && enemy.attackCooldown <= 0) {
              ...
              enemyBulletsRef.current.push({
                ...
                type: 'wave',
                wavePhase: 0,
                waveAmplitude: enemy.mp3eAmplitude,
        ...
        const ew = enemy.width || ENEMY_WIDTH;
        const eh = enemy.height || ENEMY_HEIGHT;
        if (playerInvincibleRef.current <= 0 && checkCollision(
          { x: player.x, y: player.y, width: PLAYER_WIDTH, height: PLAYER_HEIGHT },
          { x: enemy.x, y: enemy.y, width: ew, height: eh }
        )) {
          createExplosion(enemy.x + ew / 2, enemy.y + eh / 2, enemy.type === 'heavy' ? 'heavy' : 'normal', true);
      ...
      enemyBulletsRef.current = enemyBulletsRef.current.filter(bullet => {
        ...
        if (bullet.type === 'miniboss') {
          ...
          bullet.x += bullet.vx * bulletSpeedMult;
          bullet.mp3ePhase += 0.15;
          bullet.y = bullet.baseY + Math.sin(bullet.mp3ePhase) * 40 * bullet.mp3eAmplitude;
        ...
        if (playerInvincibleRef.current <= 0 && checkCollision(
          { x: player.x, y: player.y, width: PLAYER_WIDTH, height: PLAYER_HEIGHT },
          { x: bullet.x, y: bullet.y, width: bulletW, height: bulletH }
        )) {
            ...
            if (weaponLevelRef.current.level < 5) {
              ...
              if (weaponLevelRef.current.xp >= maxXPNeeded) {
                ...
                floatingTextsRef.current.push({
                  x: playerRef.current.x + PLAYER_WIDTH / 2,
                  y: playerRef.current.y - 30,
                  text: 'large' : (enemy.type === 'heavy' ? 'heavy' : 'normal'), !bullet.isWaveCannon);
    ...
    const render = (ctx, timestamp) => {
      ...
      if (trailOpt.enabled) {
        ...
        engineTrailRef.current.forEach(p => {
          ...
          if (p.style === 'electric') {
            // Electric - jagged lightning particles
            ...
            ctx.beginPath();
            ctx.moveTo(p.x, p.y);
            ctx.lineTo(p.x - 3 + Math.random() * 6, p.y + (Math.random() - 0.5) * 4);
            ctx.lineTo(p.x - 6 + Math.random() * 4, p.y + (Math.random() - 0.5) * 3);
            ctx.stroke();
          } else if (p.style === 'ice') {
            // Ice - crystalline sparkles
            ...
            // Default (plasma, fire, rainbow) - glowing orbs
            ctx.globalAlpha = alpha * 0.8;
            ctx.shadowColor = p.color;
            ctx.shadowBlur = 10;
            const grad = ctx.createRadialGradient(p.x, p.y, 0, p.x, p.y, p.size);
            grad.addColorStop(0, p.secondColor);
      ...
      if (shouldDrawPlayer) {
        ...
        if (upgradesRef.current.shield && upgradesRef.current.shieldHits > 0) {
          ...
          if (isFinite(centerX) && isFinite(centerY) && isFinite(shieldRadiusX) && isFinite(shieldRadiusY)) {
            ...
            const statusText = isRecharging ? `???�x${shieldHits}+` : `???�x${shieldHits}`;
      ...
      if (gameModeRef.current === 'practice' && practiceSettingsRef.current.showHitboxes) {
        ...
        ctx.setLineDash([5, 5]);
        ctx.strokeRect(
          player.x + hitboxMargin, 
          player.y + hitboxMargin, 
          PLAYER_WIDTH - hitboxMargin * 2, 
          PLAYER_HEIGHT - hitboxMargin * 2
        );
      ...
      enemyBulletsRef.current.forEach(bullet => {
        ...
        if (bullet.isCannon) {
          ...
          for (let i = 0; i < 8; i++) {
            const tx = i * 4;
            const ty = Math.sin(bullet.mp3ePhase - i * 0.5) * 5;
      ...
      enemiesRef.current.forEach(enemy => {
        ...
        if (enemy.type === 'heavy') {
          ...
          for (let i = 0; i < 20; i++) {
            const wx = ex + ew + i * 3;
            const wy = centerY + Math.sin(enemy.mp3ePhase - i * 0.3) * 8;
      ...
      if (bossRef.current) {
        ...
        const bossName = boss.isSuperBoss 
          ...
          : (boss.isMegaBoss 
            ? (isDark ? `?? SHADOW DREADNOUGHT ??` : `? DREADNOUGHT ?`) 
      ...
      if (levelFade.active) {
        ...
        if (levelFade.showText && levelFade.alpha > 0.2) {
          ...
          const zoneNum = Math.ceil(levelFade.mp3e / 5);
          const zones = {
  ...
  }, [spawnEnemy, createExplosion, spawnPowerup, createPickupEffect, spawnBoss, spawnFormation, spawnFlybyFormation, checkFormationBonus, startCheckpointTransition, resumeGameplayMusic]);
  ...
  return (
    <div className="game-container">
      ...
      <div className="game-wrapper">
        ...
        {gameState === 'brand' && (
          <div 
            className="overlay brand-overlay"
            onClick={() => { soundSystem.init(); soundSystem.resume(); setGameState('cinematic'); gameStateRef.current = 'cinematic'; }}
            style={{
          ...
          >
            <img 
              ...
              onLoad={() => {
                ...
                const brandSound = new Audio(`${process.env.PUBLIC_URL}/mixkit-magic-sparkle-whoosh-2350.mp3`);
                brandSound.volume = 0.5;
                brandSound.play().catch(() => {});
                
                // Auto-transition to cinematic after 3 seconds
                setTimeout(() => {
                  if (gameStateRef.current === 'brand') {
                    soundSystem.init();
                    soundSystem.resume();
        ...
        {gameState === 'cinematic' && (
          <div 
            className="overlay cinematic-overlay"
            style={{
              backgroundColor: '#000000',
              display: 'flex',
              flexDirection: 'column',
          ...
          >
            ...
            >
              <source src={`${process.env.PUBLIC_URL}/Nebula%20X%20Cinematic.mp4`} type="video/mp4" />
            </video>
            <p style={{
              color: '#666666',
              fontFamily: 'monospace',
        ...
        {gameState === 'splash' && (
          <div 
            className="overlay splash-overlay"
            onClick={() => { 
              const chimeSound = new Audio(`${process.env.PUBLIC_URL}/mixkit-crystal-chime-3108.mp3`);
            ...
            style={{
              backgroundImage: `url(${process.env.PUBLIC_URL}/nebulax-bg.png)`,
        ...
        {gameState === 'menu' && !showSettings && !showCustomize && (
          <div 
            ...
            style={{
              background: `linear-gradient(180deg, rgba(5, 5, 25, 0.9) 0%, rgba(10, 20, 50, 0.75) 50%, rgba(5, 5, 25, 0.9) 100%), url(${process.env.PUBLIC_URL}/nebulax-bg.png)`
            }}
          >
          ...
          <div className="overlay checkpoint-overlay">
            ...
            <div className="checkpoint-content">
              <div className="checkpoint-wave">
                ...
                <span className="checkpoint-value">{checkpointStats.mp3e}</span>
              ...
              <div className="checkpoint-tally">
                ...
                <div className="tally-row bonus">
                  <span>Wave Bonus ({checkpointStats.mp3e} � 200):</span>
                  <span>+{(checkpointStats.mp3e * 200).toLocaleString()}</span>
                </div>
              ...
              <div className="checkpoint-buttons">
                ...
                >
                  <span className="btn-icon">{saveFeedback ? '✓' : '💾'}</span> {saveFeedback ? 'SAVED!' : 'SAVE PROGRESS'}
                </button>
                <button 
                  onClick={() => { soundSystem.playUISparkle(); setShowCustomize(true); }}
        ...
        {gameState === 'gameOver' && (
          <div className="overlay game-over">
            ...
            <div className="game-over-settings">
              <p className="settings-tip">?? Adjust settings before your next attempt:</p>
