...
// Check if WASM is available (set this to false during development if you don't have WASM compiled)
const WASM_ENABLED = false; // Set to true after compiling WASM with build.bat
...
export async function loadPhysicsModule() {
  ...
  loadPromise = (async () => {
    try {
      // Skip WASM loading if disabled or not in browser
      if (!WASM_ENABLED || typeof window === 'undefined') {
        throw new Error('WASM disabled or not in browser environment');
      }

      // Dynamically construct the import to prevent Vite from analyzing it
      // Note: This will only work if game_physics.js exists in public/
      // The file is generated by running: cd wasm && build.bat
      const importPath = '/game_physics.js';
      const wasmModule = await new Function('path', 'return import(path)')(importPath);

      // Initialize the WASM module
      physicsModule = await wasmModule.default();
